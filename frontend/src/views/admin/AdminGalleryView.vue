...
const fetchItems = async () => {
  ...
  try {
    const response = await axios.get('/api/gallery?limit=100')
...
const openAddModal = () => {
  modalMode.value = 'add'
  formData.value = { title: '', description: '', type: 'image', category: 'general', fileUrl: '' }
...
const saveItem = async () => {
  ...
  try {
    if (modalMode.value === 'add') {
      await axios.post('/api/gallery', formData.value, { headers })
    } else {
      await axios.put(`/api/gallery/${currentId.value}`, formData.value, { headers })
    }
    closeModal()
    fetchItems()
  } catch (err: any) {
  ...
  <div class="admin-gallery-container">
    <div class="header">
      ...
      <div>
        ...
        <button @click="openAddModal" class="add-btn">+ Add Item</button>
      </div>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="item in items" :key="item._id">
            ...
            <td>{{ item.type }}</td>
            <td>{{ item.category }}</td>
            <td>{{ item.fileUrl?.substring(0, 30) }}...</td>
            <td>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="saveItem">
          ...
          <div class="form-group">
            ...
            <input v-model="formData.fileUrl" required />
          </div>
...
<style scoped>
.admin-gallery-container { padding: 2rem; background-color: #f9fafb; min-height: 100vh; }
