...
const fetchItems = async () => {
  ...
  try {
    ...
    items.value = response.data.articles || response.data
  } catch (err: any) {
...
const openAddModal = () => {
  modalMode.value = 'add'
  formData.value = { title: '', content: '', category: 'announcement', status: 'draft' }
...
const saveItem = async () => {
  ...
  try {
    if (modalMode.value === 'add') {
      await axios.post('/api/news', formData.value, { headers })
    } else {
      await axios.put(`/api/news/${currentId.value}`, formData.value, { headers })
    }
    closeModal()
    fetchItems()
  } catch (err: any) {
  ...
  <div class="admin-news-container">
    <div class="header">
      ...
      <div>
        ...
        <button @click="openAddModal" class="add-btn">+ Add News</button>
      </div>
    </div>
...
<style scoped>
.admin-news-container { padding: 2rem; background-color: #f9fafb; min-height: 100vh; }
