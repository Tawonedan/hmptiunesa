...
const openAddModal = () => {
  modalMode.value = 'add'
  formData.value = { name: '', studentId: '', email: '', yearJoined: new Date().getFullYear(), batch: '', department: '' }
...
const saveItem = async () => {
  ...
  try {
    if (modalMode.value === 'add') {
      await axios.post('/api/members', formData.value, { headers })
    } else {
      await axios.put(`/api/members/${currentId.value}`, formData.value, { headers })
    }
    closeModal()
    fetchItems()
  } catch (err: any) {
...
<style scoped>
.admin-members-container { padding: 2rem; background-color: #f9fafb; min-height: 100vh; }
